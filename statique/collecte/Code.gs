/**
 * Collecte du diagnostic Manipulation d'équations.
 * Coller equations/teacher/Code-a-coller.gs (généré).
 */

var COPIES = "Copies";
var ITEMS = "Items";
var DOMAINES = ["ACT", "LIA", "MOM", "EQU", "RDM", "MOD"];
...
function setup() {
  ...
  if (copies.getLastRow() === 0) {
    copies.appendRow(
      ["recu_at", "session", "nom", "prenom", "groupe", "bac", "duree_s"]
        .concat(DOMAINES)
        .concat(["appui", "priorites", "json"])
    );
...
function doGet() {
  return json_({ ok: true, service: "diagnostic-statique-bts-ms", session: "2026-statique" });
}
...
function doPost(e) {
  try {
    ...
    ecrire_(data, profil, cle);
    return json_({
      ok: true,
      schema: "diagnostic-statique-r1-v1",
...
function scorer_(copie, cle) {
  var items = cle.items;
  var ordre = cle.ordre_domaines || DOMAINES;
  var tot = {}, ok = {}, details = [];
  ...
  return {
    scores: scores, niveaux: niveaux, appui: appui, priorites: priorites,
    details: details, message: message_(cle, appui, priorites),
  };
}
...
function ecrire_(copie, profil, cle) {
  setup();
  var ss = SpreadsheetApp.getActive();
  var ident = copie.identite;
  var recu = new Date();
  var ordre = (cle && cle.ordre_domaines) || DOMAINES;
  var ligne = [
    recu, copie.session || "", ident.nom, ident.prenom, ident.groupe, ident.bac,
    copie.duree_totale_s || "",
  ];
  ordre.forEach(function (d) { ligne.push(profil.scores[d]); });
  ligne.push(profil.appui.join(" "), profil.priorites.join(" "), JSON.stringify(copie));
  ss.getSheetByName(COPIES).appendRow(ligne);
...
function sameSet_(a, b) {
  ...
  return a.slice().sort().join("\0") === b.slice().sort().join("\0");
}
