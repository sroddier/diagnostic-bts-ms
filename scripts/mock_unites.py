# -*- coding: utf-8 -*-
"""Collecte locale du diagnostic unités — port 8788, code UN26R1."""
...
EXPORT = ROOT / "unites" / "teacher" / "exports"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        self._json(200, {"ok": True, "service": "diagnostic-unites-local"})
    ...
    def do_POST(self):
        ...
        res["message"] = (
            "Point d’appui : "
            + ", ".join(noms[d]["nom"] for d in res["appui"][:2])
            + ". Priorité de travail : "
            + ", ".join(noms[d]["nom"] for d in res["priorites"][:2])
            + "."
        )
...
if __name__ == "__main__":
    ...
    print(f"Collecte unités  http://127.0.0.1:{port}  code {SESSION_CODE}")
